...
from .models import NotificationBase, NotificationSingle, NotificationStatus, NotificationPeriodicity, NotificationType
...
class NotificationSingleTests(TestCase):
    def setUp(self):
        self.username = 'admin_name_single'
        self.email = 'admin_email@gmail.com'
        self.password = 'admin_single'
        ...
        self.notification_test_type = 'test single type'
        self.test_color = '#f8f8f8'    
        ...
        self.type_single = NotificationBase.objects.create(
            user=self.basic_user,
        )

        self.single_notification = NotificationSingle.objects.create(
            id=1,
            notification_task_type=self.notif_type,
            text=self.test_text,
            notification_date=(self.now).date(),
            notification_time=(self.now).time(),
            notification_status=self.notification_status,
            notification_type_single=self.type_single
        )

    def test_check_creating_notification_single(self):
        ''' Проверка создания модели NotificationSingle'''
        notification_singles = NotificationSingle.objects.all().count()
        self.assertEqual(notification_singles, 1)

    def test_get_notification_single_data(self):
        '''Проверка получения данных у Single Notification'''
        notification_single = NotificationSingle.objects.get(id=1)
        self.assertEqual(notification_single.notification_task_type, self.notif_type)
        self.assertEqual(notification_single.text, self.test_text)
        self.assertEqual(notification_single.notification_date, self.now.date())
        self.assertEqual(notification_single.notification_time, self.now.time())
        self.assertEqual(notification_single.notification_status, self.notification_status)
        self.assertEqual(notification_single.notification_type_single, self.type_single)
    ...
    def test_notification_date_isnot_earlier_than_created(self):
        ...
        notification_single = NotificationSingle.objects.get(id=1)
        notification_time_and_date = datetime.combine(notification_single.notification_date, notification_single.notification_time)
        notification_time_and_date_past = notification_time_and_date - timedelta(days=5)
        
        # если дата напоминания > чем дата создания
        self.assertTrue(notification_time_and_date.replace(microsecond=0) >= timezone.make_naive(notification_single.notification_type_single.created_time).replace(microsecond=0))

        # # если дата напоминания < дата создания
        self.assertTrue(notification_time_and_date_past.replace(microsecond=0) < timezone.make_naive(notification_single.notification_type_single.created_time).replace(microsecond=0))


class NotificationPeriodicTests(TestCase):
    def setUp(self):
        self.username = 'admin_name_periodic'
        self.email = 'admin_email@gmail.com'
        self.password = 'admin_periodic'
        self.basic_user = MyUser(username=self.username, email=self.email)
        self.basic_user.set_password(self.password)
        self.basic_user.save()
        self.now = datetime.now()
        ...
        self.test_color = '#f8f8f8'    
        self.test_text = 'test text'
        self.notification_periodicity_num = 3
        self.frequency_hours = 4
        self.frequency_days = 15
        self.frequency_months = 0
        self.notification_status = NotificationStatus.objects.create()
        self.notification_status.save()
        self.notif_type = NotificationType.objects.create(
            user=self.basic_user,
        ...
        self.type_periodic = NotificationBase.objects.create(
            user=self.basic_user,
            created_time=self.now
        )

        self.periodic_notification = NotificationPeriodicity.objects.create(
            id=1,
            notification_task_type=self.notif_type,
            text=self.test_text,
            notification_periodicity_num=self.notification_periodicity_num,
            frequency_hours=self.frequency_hours,
            frequency_days=self.frequency_days,
            frequency_months=self.frequency_months,
            notification_type_periodicity=self.type_periodic
        )
        self.periodic_notification.notification_status.add(self.notification_status)
        self.periodic_notification.save()

    def test_check_creating_notification_periodic(self):
        ''' Проверка создания модели NotificationPeriodic'''
        notification_periodic = NotificationPeriodicity.objects.all().count()
        self.assertEqual(notification_periodic, 1)

    def test_get_notification_periodic_data(self):
        '''Проверка получения данных у Periodic Notification'''
        notification_periodic = NotificationPeriodicity.objects.get(id=1)
        self.assertEqual(notification_periodic.notification_task_type, self.notif_type)
        self.assertEqual(notification_periodic.text, self.test_text)
        self.assertEqual(notification_periodic.notification_periodicity_num, self.notification_periodicity_num)
        self.assertEqual(notification_periodic.frequency_hours, self.frequency_hours)
        self.assertEqual(notification_periodic.frequency_days, self.frequency_days)
        self.assertEqual(notification_periodic.frequency_months, self.frequency_months)
        self.assertEqual(notification_periodic.notification_type_periodicity, self.type_periodic)
        self.assertEqual(notification_periodic.notification_status.get(id=2), self.notification_status)
